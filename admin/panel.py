...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id in ADMIN_USER_IDS:
        ...
        await context.bot.send_photo(
            ...
            photo=
            "https://files.catbox.moe/x79o1v.jpg",  # Replace with the actual photo URL
...
async def users_menu(update: Update,
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.message.edit_text("Users Menu:",
                                                reply_markup=reply_markup)
    return USERS_MENU


async def referrals_menu(update: Update,
                         context: ContextTypes.DEFAULT_TYPE) -> int:
    """Display the referrals menu."""
    keyboard = [[InlineKeyboardButton("Back", callback_data='back_to_admin')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.message.edit_text("Referrals Menu:",
                                                reply_markup=reply_markup)
    return REFERRALS_MENU


async def shop_menu(update: Update,
                   context: ContextTypes.DEFAULT_TYPE) -> int:
    """Display the shop menu."""
    keyboard = [[InlineKeyboardButton("Back", callback_data='back_to_admin')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.message.edit_text("Shop Menu:",
                                                reply_markup=reply_markup)
    return SHOP_MENU


async def broadcast_menu(update: Update,
                        context: ContextTypes.DEFAULT_TYPE) -> int:
    """Display the broadcast menu."""
    keyboard = [[InlineKeyboardButton("Back", callback_data='back_to_admin')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.callback_query.message.edit_text("Broadcast Menu:",
                                                reply_markup=reply_markup)
    return BROADCAST_MENU


async def back_to_admin_menu(update: Update,
                             context: ContextTypes.DEFAULT_TYPE) -> int:
...
async def view_users_handler(update: Update,
                             context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle the 'View Users' button."""
    # Add your implementation here to fetch and display users from the database
    # For now, using placeholder data
    message = "Here's a list of users:\n\nUser 1\nUser 2\nUser 3"
    await update.callback_query.message.edit_text(message)
    return USERS_MENU  # Stay in the USERS_MENU state


async def edit_user_handler(update: Update,
                            context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle the 'Edit User' button."""
    # Add your implementation here to edit user details
    # For now, using placeholder data
    message = "Editing user... (Not implemented yet)"
    await update.callback_query.message.edit_text(message)
    return USERS_MENU  # Stay in the USERS_MENU state
